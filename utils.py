...
def pad_sents_char(sents, char_pad_token):
    ...
    sentence_lengths = [len(s) for s in sents] # This is the length of each sentence in the batch
    max_sentence_length = max(sentence_lengths) # Pad to this length - the longest sentence in the batch

    # The input, sents, has a list of elements that look like this [[1, 30, 2], [1, 31, 2], [1, 32, 70, 2]]

    sents_padded = []
    for s in sents:
        while len(s) < max_sentence_length:
            s.append([char_pad_token] * max_word_length)
        for w in s:
            while len(w) < max_word_length:
                w.append(char_pad_token)
            if len(w) > max_word_length:
                w = w[:max_word_length] # Truncatation
