...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        # def prepend(word):
        #     wordlist.insert(0, self.start_of_word)
        #     wordlist.append(self.end_of_word)
        #     return wordlist

        def prepend(word):
            return '{' + word + '}'

        # Need to apply prepend to each word
        prepended_sents = [[prepend(w) for w in s] for s in sents]
        result = [[[self.char2id[c] for c in w] for w in s] for s in prepended_sents]
        return result
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_t = pad_sents_char(char_ids, self.word2id['<pad>'])
        sents_var = torch.tensor(sents_t, dtype=torch.long, device=device) # Shape (batch_size, max_sentence_length, max_word_length)
        sents_var = sents_var.permute([1,0,2]) # Shape(max_sentence_length, batch_size, max_word_length)
